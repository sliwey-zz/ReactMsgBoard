	...
	loadMore: function() {
		...
		promise.done(function(data) {
			if (data && data.length > 0) {
				...
				that.setState({
					data: newComments,
					moreLoading: "loading hide"
				});
				that.flag = true;
			} else {
				that.setState({noMore: "no-more show"});
				that.flag = false;
			}
		});
	...
	render: function() {
		return (
			React.createElement("div", {className: "comment-box"}, 
				...
				React.createElement("p", {className: this.state.addLoading}, "Loading..."), 
				React.createElement(CommentList, {data: this.state.data}), 
				React.createElement("p", {className: this.state.moreLoading}, "Loading..."), 
